...
def update_me(
    ...
    crud_user.update_user(db, user_schema, current_user)
    return current_user
...
@router.post("/open/users/", response_model=UserSchema)
...
@router.get("/open/users/", response_model=List[UserSchema])
def read_open_users(
    skip: int = 0, # GETパラメータ
    limit: int = 100, # GETパラメータ
    db: Session = Depends(db.get_db),
...
@router.get("/open/users/{user_id}", response_model=UserSchema)
def read_open_user(
    user_id: int, # URLの{user_id}プレースホルダ
    db: Session = Depends(db.get_db),
):
...
@router.put("/open/users/{user_id}", response_model=UserSchema)
def update_open_user(
    user_id: int,
    user_schema: UserCreateSchema,
    db: Session = Depends(db.get_db),
):
...
@router.delete("/open/users/{user_id}")
def delete_open_user(
    user_id: int,
    db: Session = Depends(db.get_db),
):
    user = crud_user.get_user(db, user_id=user_id)
...
@router.post("/open/users/{user_id}/roles/{role_id}", response_model=UserSchema)
def associate_open_user_role(
    user_id: int,
    role_id: int,
    db: Session = Depends(db.get_db),
):
